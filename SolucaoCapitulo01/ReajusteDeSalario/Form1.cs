...
namespace ReajusteDeSalario
{
    public partial class frmReajusteSalarial : Form
    {
        ...
        private void TotalizarValores(IList<Funcionario> dadosLidos)
        {
            ...
            lblValorTotalSemRea.Text = string.Format("{0:c}", totalSemReajuste);
            lblValorTotalReajuste.Text = string.Format("{0:c}", totalComReajuste);
            lblValorPercenReajuste.Text = string.Format("{0:n}%", percentualReajuste);
        }
        ...
        private void ProcessarArquivo(string nomeArquivo)
        {
            ...
            string linhaLida;
            var arquivo = new System.IO.StreamReader(nomeArquivo);//abre o arquivo
